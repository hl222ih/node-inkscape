...
class Inkscape extends Stream {
  constructor(inkscapeArgs) {
    ...
    if (
      ...
      //this.inkscapeArgs.push('--without-gui'); //Avoid "Unknown option --without-gui" crash in Inkscape 1.0
    }
    ...
    if (!this.outputFormat && !this.guiMode) {
      ...
      this.inkscapeArgs.push(`--export-filename=${this.inkscapeOutputFilePath}`);
    }
